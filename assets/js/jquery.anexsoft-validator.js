jQuery.fn.validate = function ()
{
    ...
    var _mensaje = {
        ...
        campo_nombre: 'Este campo no es un nombre propio',
        campo_curp: 'Este campo no es una curp',
            ...
            if (!obj.prop('readonly') || !obj.prop('disabled'))
            {
                if ($(this).data('validacion-tipo') != undefined) {
                    ...
                    $.each($(this).data('validacion-tipo').split('|'), function (i, v) {
                        ...
                        /* Validamos si es una CURP */
                        if (v == 'curp') {
                            if (!obj.val().match(/^([A-Z]{4}([0-9]{2})(0[1-9]|1[0-2])(0[1-9]|1[0-9]|2[0-9]|3[0-1])[HM](AS|BC|BS|CC|CL|CM|CS|CH|DF|DG|GT|GR|HG|JC|MC|MN|MS|NT|NL|OC|PL|QT|QR|SP|SL|SR|TC|TS|TL|VZ|YN|ZS|NE)[A-Z]{3}[0-9A-Z]\d)$/i) && obj.val().length > 0) {
                                errores++;
                                form_group.addClass('has-error');
                                /* Mostramos el mensaje */
                                if (obj.data('validacion-mensaje') == undefined) {
                        ...
                        /* Validamos si es nombre */
                        if (v == 'nombre') {
                            if (!obj.val().match(/^([A-ZÁÉÍÓÚ]{1}[a-zñáéíóú]+[\s]*)+$/) && obj.val().length > 0) {
                                ...
                                /* Mostramos el mensaje */
                                if (obj.data('validacion-mensaje') == undefined) {
                                    small.text(_mensaje.campo_nombre);
                                } else {
                                    small.text(obj.data('validacion-mensaje'));
                                }
